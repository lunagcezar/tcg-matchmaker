  ...
  <q-page class="q-pa-md">
    ...
    <AdminTable :rows="reports" :columns="columns" :loading="loading">
      <template #body-cell-status="{ row }">
        <q-td>
          <q-badge :color="row.status === 'pending' ? 'warning' : row.status === 'resolved' ? 'positive' : 'grey'">{{ row.status }}</q-badge>
        </q-td>
      </template>
...
async function fetchReports() {
  loading.value = true;
  try { const r = await fetch(`${apiUrl}/api/reports`); const b = await r.json(); reports.value = b.data ?? []; } finally { loading.value = false; }
}
async function resolveReport(id: string) { await fetch(`${apiUrl}/api/reports/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'resolved' }) }); await fetchReports(); }
async function dismissReport(id: string) { await fetch(`${apiUrl}/api/reports/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'dismissed' }) }); await fetchReports(); }
