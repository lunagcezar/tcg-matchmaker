    ...
    <div v-if="session" style="width: 600px">
      <q-card>
        <q-card-section>
          ...
          <q-badge :color="statusColor(session.status)" class="q-mt-sm">{{ session.status }}</q-badge>
        </q-card-section>
        <q-card-section>
        ...
        <q-card-actions class="q-pa-md q-gutter-sm">
          <q-btn v-if="session.status === 'planned' || session.status === 'active'" color="primary" :label="$t('event.rsvp')" @click="rsvp" :loading="rsvping" :disable="!!myParticipation" />
...
function statusColor(s: string | undefined) { return s === 'active' ? 'positive' : s === 'planned' ? 'primary' : s === 'cancelled' ? 'negative' : 'grey'; }

async function rsvp() {
  rsvping.value = true;
  try { await store.join(sessionId); await loadData(); } finally { rsvping.value = false; }
}

async function confirmAttendance() {
  confirming.value = true;
  try { await store.confirm(sessionId); await loadData(); } finally { confirming.value = false; }
}

async function declineAttendance() {
  declining.value = true;
  try { await store.decline(sessionId); await loadData(); } finally { declining.value = false; }
}
