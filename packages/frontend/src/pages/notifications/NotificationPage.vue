<template>
  <q-page class="q-pa-md">
    <div class="row items-center q-mb-md">
      ...
      <q-space />
      <q-btn :label="$t('notifications.markAllRead')" flat color="primary" @click="handleMarkAllRead" :disable="unreadCount === 0" />
    </div>

    <q-list bordered separator v-if="notifications.length > 0">
      <q-item v-for="n in notifications" :key="n.id" clickable @click="handleClick(n)" :class="{ 'bg-grey-1': !n.read_at }">
        ...
        <q-item-section>
          ...
          <q-item-label caption class="text-caption text-grey">{{ formattedDate(n.created_at) }}</q-item-label>
        </q-item-section>
        <q-item-section side v-if="!n.read_at">
          <q-badge rounded color="primary" size="sm" />
        </q-item-section>
        <q-item-section side>
          <q-btn flat dense round icon="done" size="sm" @click.stop="markAsRead(n.id)" v-if="!n.read_at" />
        </q-item-section>
      </q-item>
    </q-list>
