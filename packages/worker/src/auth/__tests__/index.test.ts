...
function createTestApp() {
  return new Hono<{ Bindings: typeof env; Variables: { user: { id: string; email: string; username: string; role: string } } }>()
    ...
    it("returns user profile when authenticated", async () => {
      ...
      const c = chain({
        single: vi.fn().mockResolvedValue({
          data: {
            id: testUserId, username: "testuser", display_name: "Test User",
            role: "player", avatar_path: null, banned_at: null,
            suspended_at: null, created_at: "2026-01-01T00:00:00.000Z",
          }, error: null,
        }),
      });
      ...
      const res = await createTestApp().request("/api/auth/me", { headers: { Authorization: "Bearer t" } }, env);
