...
tournamentRouter.get('/:id', async (c) => {
  ...
  const { count } = await supabase
    ...
    .eq('event_id', data.id);
  return c.json({
    data: { ...EventSchema.parse(data), participant_count: count ?? 0 },
    error: null,
    meta: null,
  });
});
...
tournamentRouter.patch('/:id', authMiddleware, async (c) => {
  ...
  const { data: existing } = await supabase
    ...
    .eq('id', c.req.param('id'))
    .single();
  if (!existing || !orgGuard(user.id, existing.created_by_user_id))
...
tournamentRouter.post('/:id/publish', authMiddleware, async (c) => {
  ...
  if (existing.status !== 'draft')
    return c.json({ data: null, error: 'Tournament must be in draft status', meta: null }, 400);
...
tournamentRouter.post('/:id/register', authMiddleware, async (c) => {
  ...
  if (!event || event.status !== 'open')
    return c.json(
      { data: null, error: 'Tournament is not open for registration', meta: null },
      400,
    );
  ...
  if (event.max_participants) {
    ...
    if ((count ?? 0) >= event.max_participants)
      return c.json({ data: null, error: 'Tournament is full', meta: null }, 400);
  }

  const { data: participant } = await supabase
    .from('event_participants')
    .insert({ event_id: event.id, user_id: user.id, role: 'participant', status: 'pending' })
    .select()
    .single();
...
tournamentRouter.post('/:id/start', authMiddleware, async (c) => {
  ...
  const { data: existing } = await supabase
    .from('events')
    .select('created_by_user_id, status, bracket_type')
    .eq('id', c.req.param('id'))
    .single();
  if (!existing || !orgGuard(user.id, existing.created_by_user_id))
    return c.json({ data: null, error: 'Forbidden', meta: null }, 403);
  if (existing.status !== 'open')
    return c.json({ data: null, error: 'Tournament must be open to start', meta: null }, 400);
  ...
  await generator(supabase, c.req.param('id')!, ids);
...
tournamentRouter.get('/:id/bracket', async (c) => {
  ...
  const { data: matches } = await supabase
    ...
    .order('id');
  return c.json({
    data: { rounds: rounds ?? [], matches: matches ?? [] },
    error: null,
    meta: null,
  });
});
...
bracketMatchRouter.post('/:id/report', authMiddleware, async (c) => {
  ...
  if (!match || match.status === 'completed' || match.status === 'walkover')
    return c.json({ data: null, error: 'Match not found or already completed', meta: null }, 404);
