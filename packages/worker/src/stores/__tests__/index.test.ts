...
const storeData = {
  ...
  description: null, country: "Brasil", state: "Ceará", city: "Fortaleza",
  address: "Rua Teste, 123", lat: -3.7, lng: -38.5,
  phone: null, website: null, logo_path: null,
  created_by_user_id: testUserId, is_verified: false, status: "active",
...
describe("Store routes", () => {
  ...
  describe("POST /api/stores", () => {
    it("returns 401 when not authenticated", async () => {
      ...
      const res = await makeApp().route("/api/stores", storeRouter).request("/api/stores", { method: "POST" }, env);
      expect(res.status).toBe(401);
    });

    it("creates a store and returns 201", async () => {
      const { createClient } = await import("@supabase/supabase-js");
      const sChain = chain({ single: vi.fn().mockResolvedValue({ data: storeData, error: null }), insert: vi.fn().mockReturnThis() });
      const mChain = chain();
      ...
      const res = await makeApp().route("/api/stores", storeRouter).request("/api/stores", {
  ...
  describe("PATCH /api/stores/:id", () => {
    it("returns 403 when user is not a member", async () => {
      ...
      (createClient as ReturnType<typeof vi.fn>).mockReturnValue({ ...authMock(testUserId2), from: vi.fn().mockImplementation((t: string) => {
        if (t === "users") return userChain(testUserId2);
        if (t === "store_memberships") return mChain;
        return chain();
      }) });

      const res = await makeApp().route("/api/stores", storeRouter).request(`/api/stores/${storeId}`, {
  ...
  describe("POST /api/stores/:id/verify", () => {
    it("returns 403 when user is not admin", async () => {
      ...
      const res = await makeApp().route("/api/stores", storeRouter).request(`/api/stores/${storeId}/verify`, {
  ...
  describe("GET /api/stores/:id/members", () => {
    it("returns list of members", async () => {
      const { createClient } = await import("@supabase/supabase-js");
      const mChain = chain({ maybeSingle: vi.fn().mockResolvedValue({ data: { store_id: storeId, user_id: testUserId, role: "owner" }, error: null }) });
      ...
      const res = await makeApp().route("/api/stores", storeRouter).request(`/api/stores/${storeId}/members`, {
