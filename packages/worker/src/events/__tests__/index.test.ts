...
const eventData = {
  id: eventId, type: "match", created_by_user_id: testUserId,
  organizer_user_id: testUserId, organizer_store_id: null,
  ...
  max_participants: null, bracket_type: null,
...
describe("Event routes", () => {
  ...
  describe("POST /api/events", () => {
    it("returns 401 when not authenticated", async () => {
      ...
      const res = await makeApp().route("/api/events", eventRouter).request("/api/events", { method: "POST" }, env);
      expect(res.status).toBe(401);
    });

    it("creates a match and returns 201", async () => {
      const { createClient } = await import("@supabase/supabase-js");
      const eChain = chain({ single: vi.fn().mockResolvedValue({ data: eventData, error: null }), insert: vi.fn().mockReturnThis() });
      ...
      const res = await makeApp().route("/api/events", eventRouter).request("/api/events", {
        method: "POST", headers: { Authorization: "Bearer t", "Content-Type": "application/json" },
        body: JSON.stringify({ type: "match", scheduled_at: "2026-07-20T14:00:00.000Z", lat: -3.7, lng: -38.5 }),
      }, env);
  ...
  describe("POST /api/events/:id/join", () => {
    it("returns 401 when not authenticated", async () => {
      ...
      const res = await makeApp().route("/api/events", eventRouter).request(`/api/events/${eventId}/join`, {
    ...
    it("adds participant with pending status", async () => {
      const { createClient } = await import("@supabase/supabase-js");
      const eChain = chain({ single: vi.fn().mockResolvedValue({ data: { id: eventId, type: "match", status: "open", max_participants: null, created_by_user_id: testUserId2 }, error: null }) });
      const pChain = chain({
        single: vi.fn().mockResolvedValue({ data: { id: "00000000-0000-0000-0000-000000000010", event_id: eventId, user_id: testUserId, role: "opponent", status: "pending", confirmed_at: null, score: null, placement: null, seed: null, created_at: "2026-07-14T00:00:00.000Z" }, error: null }),
  ...
  describe("DELETE /api/events/:id", () => {
    it("returns 403 when user is not the creator", async () => {
      const { createClient } = await import("@supabase/supabase-js");
      const eChain = chain({ single: vi.fn().mockResolvedValue({ data: { id: eventId, created_by_user_id: testUserId2 }, error: null }) });
      ...
      const res = await makeApp().route("/api/events", eventRouter).request(`/api/events/${eventId}`, {
